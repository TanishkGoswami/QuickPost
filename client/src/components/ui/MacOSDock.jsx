...
/**
 * MacOS-style Dock component with magnification effect.
 * Adapted for JavaScript (JSX) from TypeScript source.
 */
const MacOSDock = ({ 
  apps = [], 
  onAppClick = () => {}, 
  ...
  const animateToTarget = useCallback(() => {
    ...
    setCurrentScales(prevScales => prevScales.map((currentScale, index) => {
      const diff = targetScales[index] - currentScale;
      return currentScale + (diff * lerpFactor);
    }));

    setCurrentPositions(prevPositions => prevPositions.map((currentPos, index) => {
  ...
  const padding = Math.max(10, baseIconSize * 0.15);
            ...
            >
              <img
                ...
                style={{
                  width: '100%',
                  height: '100%',
                  filter: `drop-shadow(0 ${Math.max(2, baseIconSize * 0.04 * scale)}px ${Math.max(4, baseIconSize * 0.08 * scale)}px rgba(0,0,0,${0.15 + (scale - 1) * 0.1}))`
                }}
              />

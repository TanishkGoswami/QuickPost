...
function CustomDialog({ type, title, message, confirmText, cancelText, intent, onClose }) {
  const getIntentIcon = () => {
    switch (intent) {
      case 'danger': return <Trash2 className="w-10 h-10 text-red-500" />;
      case 'warning': return <AlertTriangle className="w-10 h-10 text-yellow-500" />;
      case 'logout': return <LogOut className="w-10 h-10 text-blue-500" />;
